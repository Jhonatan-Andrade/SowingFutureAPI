...
import { UserLogin, UserSignup, UserProfile,UserRepository } from "./user.entities.js";  
import { UserRepositoryDb } from "./user.repository.js";
import { isValidEmail, isValidPassword } from "../../utils/isValidLogin.js";
import { ApiError } from "../../error.js";
import bcrypt from "bcrypt";
import { signUser } from "../../middlewares/isAuthenticated.js";
...
class UserServices {
    ...
    async createUser(data: UserSignup): Promise<{token:string}> {
        ...
        const token = signUser(userCreated.name,userCreated.email);
        return {token};
    }
    async loginUser(data: UserLogin): Promise<{token:string}> {
        ...
        const token = signUser(userPassword.name,userPassword.email);
