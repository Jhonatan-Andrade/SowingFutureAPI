...
import { GoalsCreate, GoalsHistoryDataAdd } from "./goals.entities.js";
import { GoalsServices} from "./goals.services.js";
import { isAuthenticated } from "../../middlewares/isAuthenticated.js";
import { deleteGoalSchema, goalAddMoneySchema, registerGoalSchema, searchGoalSchema } from "./routeSchema.js";
...
export async function GoalsControllers(app: FastifyInstance) {
    ...
    app.post('/goals/addMoney', { 
        preHandler: isAuthenticated ,
        schema:goalAddMoneySchema
    }, async (request, reply) => {
        const email = request.user.email;
        const data  = request.body as GoalsHistoryDataAdd
      
        await goalsServices.addMoneyGoals(email,data)

        reply.status(201).send({message: 'Goals update successfully'});
    });
    app.get('/goals', { 
        preHandler: isAuthenticated ,
